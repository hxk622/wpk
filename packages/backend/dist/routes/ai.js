...
router.post('/:gameId/analyze-hand', authenticateToken, async (req, res) => {
    try {
        ...
        // 调用AI服务分析牌局
        const analysis = aiService_1.aiService.analyzeHand(userId, gameId, holeCards, communityCards || [], betHistory || [], potSize || 0, currentBet || 0, stackSize || 0);
        res.status(200).json((0, response_1.successResponse)({ analysis }, 'AI分析牌局成功'));
    }
    catch (error) {
        res.status(400).json((0, response_1.errorResponse)(error.message || 'AI分析牌局失败', 400));
    }
});
/**
 * @swagger
...
router.get('/:gameId/suggestions', authenticateToken, async (req, res) => {
    try {
        ...
        // 调用AI服务获取建议
        const suggestion = aiService_1.aiService.getSuggestion(userId, gameId, styleStr, parsedHand, parsedCommunityCards, parsedBetHistory, parseInt(potSizeStr), parseInt(currentBetStr), parseInt(stackSizeStr));
...
router.post('/train', authenticateToken, async (req, res) => {
    try {
        ...
        // 调用AI服务进行训练
        const trainingResult = await aiService_1.aiService.trainAI({
            userId,
            ...trainingData
        });
        res.status(200).json({
...
router.post('/batch-train', authenticateToken, async (req, res) => {
    try {
        ...
        // 调用AI服务进行批量训练
        const trainingResult = await aiService_1.aiService.batchTrainAI(preparedTrainingData);
        res.status(200).json({
...
router.post('/:gameId/analyze-hand', authenticateToken, async (req, res) => {
    try {
        ...
        // 调用AI服务计算牌力
        const handStrength = aiService_1.aiService['calculateHandStrength'](holeCards, communityCards || []);
        res.status(200).json({
            message: '牌力评估成功',
...
router.post('/:gameId/calculate-odds', authenticateToken, async (req, res) => {
    try {
        ...
        // 计算底池赔率
        const potOdds = aiService_1.aiService['calculatePotOdds'](potSize || 0, currentBet || 0);
        // 计算隐含赔率
        const impliedOdds = aiService_1.aiService['calculateImpliedOdds'](potSize || 0, currentBet || 0, expectedFutureBets || 0);
        // 计算牌力（作为胜率的简化表示）
        const handStrength = aiService_1.aiService['calculateHandStrength'](holeCards, communityCards || []);
...
router.get('/opponents', authenticateToken, async (req, res) => {
    try {
        const opponents = aiService_1.aiService.getAvailableOpponents();
...
router.get('/opponent/:opponentId/hand', authenticateToken, async (req, res) => {
    try {
        ...
        const opponent = aiService_1.aiService.getOpponentById(opponentId);
        if (!opponent) {
            return res.status(404).json({ error: 'AI对手不存在' });
        }
        const hand = aiService_1.aiService.generateAIOpponentHand(opponent);
        res.status(200).json({
            success: true,
            opponentId,
            hand
...
router.post('/opponent/decision', authenticateToken, async (req, res) => {
    try {
        ...
        const opponent = aiService_1.aiService.getOpponentById(opponentState.opponentId);
        if (!opponent) {
            return res.status(404).json({ error: 'AI对手不存在' });
        }
        // 构建完整的AI对手状态
        ...
        const decision = aiService_1.aiService.makeAIDecision(fullOpponentState, communityCards, potSize, currentBet, minRaise);
        res.status(200).json({
